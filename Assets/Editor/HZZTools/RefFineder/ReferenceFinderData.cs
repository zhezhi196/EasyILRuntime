...
public class ReferenceFinderData
{
    ...
    public void CollectDependenciesInfo()
    {
        try
        {
            ...
            for (int i = 0; i < allAssets.Length; i++)
            {
                ...
                if (i % 100 == 0 &&
                    EditorUtility.DisplayCancelableProgressBar("Refresh", string.Format("Collecting {0} assets", i), (float) i / totalCount))
                {
    ...
    private void WriteToChache()
    {
        ...
        using (FileStream fs = File.OpenWrite(CACHE_PATH))
        {
            ...
            {
                //使用 Where 子句过滤目录
                int[] indexes = assetDict[guid].dependencies.Where(s => guidIndex.ContainsKey(s)).Select(s => guidIndex[s]).ToArray();
