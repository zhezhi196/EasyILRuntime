...
namespace uPLibrary.Networking.M2Mqtt
{
    ...
    public class MqttClient
    {
        ...
        public void ThreadConnect(string clientId,
            string username,
            string password)
        {
            try
        ...
        public byte InitConnect(string clientId,
            string username,
            string password)
        {
            return this.InitConnect(clientId, username, password, false, MqttMsgConnect.QOS_LEVEL_AT_MOST_ONCE, false, null, null, true, MqttMsgConnect.KEEP_ALIVE_PERIOD_DEFAULT);
        }
...
#endif
        {
            ...
            UnityEngine.Debug.LogWarning("MqttClient Close");
        ...
        {
            UnityEngine.Debug.LogWarning("MqttClient OnConnectionClosing");
        ...
        private MqttMsgBase SendReceive(byte[] msgBytes, int timeout)
        {
            ...
            {
                UnityEngine.Debug.LogError("MqttClient SendReceive Error");
        ...
        private void ReceiveThread()
        {
            ...
            while (this.isRunning)
            {
                ...
                {
                    UnityEngine.Debug.LogWarning("MqttClient ReceiveThread Exception");
        ...
        private void DispatchEventThread()
        {
            while (this.isRunning)
            {
                ...
                if (this.isRunning)
                {
                    ...
                    UnityEngine.Debug.Log(string.Format("count=[{0}][closs={1}]", this.eventQueue.Count, this.isConnectionClosing));
                    if ((this.eventQueue.Count == 0) && this.isConnectionClosing)
                    {
                        // client must close connection
                        this.Close();
