...
namespace RootMotion.FinalIK {
	...
	public class AimPoser : MonoBehaviour {
		...
		public class Pose {
			...
			public bool IsInDirection(Vector3 d,Transform trans) {
				if (direction == Vector3.zero) return false;
				if (yaw <= 0 || pitch <= 0) return false;

				//Vector3 up = trans.rotation * Vector3.up;
				Vector3 normalizedPoseDirection = direction.normalized;
				Vector3 _direction = (trans.position + trans.rotation * normalizedPoseDirection)-trans.position;
				// Yaw
				if (yaw < 180f) {
					Vector3 directionYaw = new Vector3(_direction.x, 0f, _direction.z);
					if (directionYaw == Vector3.zero) directionYaw = trans.forward;
				...
				float directionPitch = Vector3.Angle(trans.up, _direction);
				float dPitch = Vector3.Angle(trans.up, d);
		...
		public Pose GetPose(Vector3 localDirection) {
			...
			Debug.DrawLine(transform.position, transform.position + localDirection, Color.yellow);
			for (int i = 0; i < poses.Length - 1; i++) if (poses[i].IsInDirection(localDirection,transform)) return poses[i];
